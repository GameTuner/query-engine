...
def test_kpi_id_multiple_filters_and_group_bys_sql(datasource_repository, kpi_repository, app_config_repository):
    ...
    query = CohortDaySpecifics()._preprocess_warehouse_query(ChartQuery(
        app=app,
        page_id="page",
        request_id="request",
        datasource=datasource,
        kpi=kpi_repository.load_by_datasource_id(app, datasource.id)['cohort_single'],
        column_filters=[
            ColumnFilter(
        ...
        x_axis_column=ColumnReference(datasource, constants.COHORT_DAY_COLUMN_NAME)
    ).to_warehouse_query())

    assert query_builder.build(query)['x'].sql == """SELECT `app_main.user_history`.`cohort_day` AS x_axis, `app_main.user_history`.`up_string` AS group_by_1, `app_main.user_history`.`up_int` AS group_by_2, SUM(`app_main.user_history`.`up_int`) AS value
FROM `app_main.user_history`
WHERE (`app_main.user_history`.`date_` BETWEEN DATE '2022-09-01' AND DATE '2022-09-10') AND `app_main.user_history`.`up_string` = 'value' AND `app_main.user_history`.`up_int` = 1 AND `app_main.user_history`.`registration_date` BETWEEN DATE '2022-09-01' AND DATE '2022-09-05'
